...
Before tightening branch-age policies, audit whether the team can get useful feedback fast enough. If validation is slow, flaky, or too shallow to catch real defects, branch lifetime will grow again under pressure. Fix that first, apply tooling whenever possible
...
No specific release branches. If it's built from `main` then it should also release from `main`. Keep the cadence high and fix forward quickly
...
Use short lived branches to show and ask:
...
Agree about working culture. Trunk-based-development isn't only for developers. It requires participation from everyone in the value stream.
